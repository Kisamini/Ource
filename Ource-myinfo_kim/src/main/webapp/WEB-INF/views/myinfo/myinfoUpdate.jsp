...
<body>
	<div id="contents">
		<ul class="profile_menu">
			<li><a href="#" id="myinfo" class="pageMove nonPageMove">프로필 보기</a></li>
			<li><a href="#" id="myinfoUpdate" class="pageMove">프로필 수정</a></li>
		</ul>
		<article>
			<div class="myinfo profile" data-user-number="<c:out value="${myinfoUpdate.userNumber}"/>">
				<span class="columnName"><img src="images/default_profile.jpg"></span>
				<span class="profileValue"><c:out value="${myinfoUpdate.userId}"/></span>
			</div>
			<div class="myinfo">
				<span class="columnName">이름</span>
				<span class="profileValue"><c:out value="${myinfoUpdate.userName}"/></span>
			</div>
			<div class="myinfo">
				<span class="columnName">성별</span>
				<span class="profileValue">
					<c:if test="${myinfoUpdate.userGender eq 'M'}">남성</c:if>
					<c:if test="${myinfoUpdate.userGender eq 'F'}">여성</c:if>
				</span>	
			</div>
			
			<div class="myinfo">
				<span class="columnName">생년월일</span>
				<span class="profileValue"><c:out value="${myinfoUpdate.userBirth}"/></span>
			</div>
			...
			<c:forEach items="${userInfoTypeList}" var="infoType">
				...
				<c:forEach items="${myinfoUpdate.userInfoDetail}" var="userDetail">
	...
	<script>
		$(function(){
			$('#myinfo').click(function(){
					location.href="myinfo.do?userid=${param.userid}";
			});
		});
	</script>
</body>
